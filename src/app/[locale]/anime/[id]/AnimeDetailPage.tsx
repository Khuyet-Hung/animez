...
interface PageProps {
  params: Promise<{ id: string; locale: string; slug?: string | string[] }>;
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    return createSeoMetadata({
      locale,
      path: buildAnimeDetailHref(id, data.Media.title, locale),
      title,
      description,
      image,
      type: "article",
...
export default async function AnimeDetailPage({ params }: PageProps) {
  ...
  const animeUrl = getAbsoluteUrl(`/${locale}${buildAnimeDetailHref(anime.id, anime.title, locale)}`);
