      ...
      {showSuggestions && suggestions.length > 0 && (
        <div className="absolute left-0 right-0 top-full z-50 mt-1 overflow-hidden rounded-ui-sm border border-border bg-surface shadow-xl">
          {suggestions.map((anime) => (
            <Link key={anime.id} href={buildAnimeDetailHref(anime.id, anime.title, locale)} onClick={() => setShowSuggestions(false)} className="flex items-center gap-3 px-3 py-2 transition-colors hover:bg-border">
